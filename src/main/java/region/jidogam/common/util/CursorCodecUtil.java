...
public class CursorCodecUtil {
   ...
   * @param sortBy   정렬 기준
   */
  public String encodeNextCursor(GuidebookResponse lastItem, String sortBy){
    String lastId = lastItem.gid().toString();
    String lowerSortBy = !(sortBy == null) ? sortBy.toLowerCase() : "createdat";
    String lastValue;
    switch (lowerSortBy) {
      case "createdat" -> lastValue = lastItem.createdAt().toString();
      case "updatedat" -> lastValue = lastItem.title();
